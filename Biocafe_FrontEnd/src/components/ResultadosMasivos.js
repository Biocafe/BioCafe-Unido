...
const ResultadosMasivos = () => {
  ...
  const [modalShapiro, setModalShapiro] = useState(false);
  const [modalLevene, setModalLevene] = useState(false);
  const [modalAnova, setModalAnova] = useState(false);
  const [modalDuncan, setModalDuncan] = useState(false);
  ...
  const exportToImage = async () => {
    ...
    let filename = "resultados-masivos.png";
    
    if (modalShapiro || modalLevene || modalAnova || modalDuncan) {
      ...
      if (modalContent) {
        ...
        const modalType = modalShapiro ? 'shapiro' : modalLevene ? 'levene' : modalAnova ? 'anova' : 'duncan';
        filename = `informe_detallado_${modalType}.png`;
      }
    }
  ...
  const exportToPDF = async () => {
    // Si hay un modal abierto, capturar el contenido del modal
    let element = modalRef.current;
    let filename = "resultados-masivos.pdf";
    ...
    if (modalShapiro || modalLevene || modalAnova || modalDuncan) {
      ...
      if (modalContent) {
        element = modalContent;
        const modalType = modalShapiro ? 'shapiro' : modalLevene ? 'levene' : modalAnova ? 'anova' : 'duncan';
        filename = `informe_detallado_${modalType}.pdf`;
      }
    }
  ...
  const exportToExcel = () => {
    ...
    if (modalShapiro || modalLevene || modalAnova || modalDuncan) {
      ...
      if (modalContent) {
        ...
        const data = [
          ...
          ['Tipo de Prueba:', `Informe Detallado - ${modalType.toUpperCase()}`],
        ...
        XLSX.writeFile(wb, `informe_detallado_${modalType}.xlsx`);
        return;
      }
    }
    ...
    const generalData = [
      ['RESULTADOS ESTADÍSTICOS MASIVOS'],
      ['Fecha de Generación:', new Date().toLocaleDateString()],
      ['Hora de Generación:', new Date().toLocaleTimeString()],
      [''],
    ...
    XLSX.writeFile(wb, 'resultados-masivos-completo.xlsx');
  };

  const abrirModal = (tipo) => {
    switch(tipo) {
      case 'shapiro':
        setModalShapiro(true);
        break;
      case 'levene':
        setModalLevene(true);
        break;
  ...
  const cerrarModal = () => {
    setModalShapiro(false);
    setModalLevene(false);
    setModalAnova(false);
    setModalDuncan(false);
  };
  ...
  const getDuncanContent = () => {
    if (!resultados.duncan || resultados.duncan.length === 0) {
      return <div>No hay datos disponibles para la prueba de Duncan</div>;
    }
    
    // Obtener tratamientos únicos y calcular estadísticas
    const tratamientos = [...new Set(resultados.duncan.map(d => d.tratamiento))];
    
    // Simular estadísticas por tratamiento (en un caso real, estos datos vendrían del backend)
    const estadisticasPorTratamiento = tratamientos.map(tratamiento => {
      const datosTratamiento = resultados.duncan.filter(d => d.tratamiento === tratamiento);
      const media = datosTratamiento.length > 0 ? datosTratamiento[0].media : 0;
      const desviacion = Math.sqrt(media * 0.1); // Estimación simple
      ...
      <div className={styles.resultadosContainer}>
        <div ref={modalRef} className={styles.resultadosContenido}>
          ...
          {resultados.duncan && resultados.duncan.length > 0 && (
            <div className={styles.pruebaDetalle}>
              <h4>Prueba de Duncan</h4>
              <div className={styles.shapiroResultados}>
                <div className={styles.estadisticos}>
                  <span><strong>Grupos identificados: {resultados.duncan.length}</strong></span>
                </div>
                <div className={styles.resultado}>
                  <div className={styles.interpretacion}>
                    <strong>Interpretación:</strong> Se han identificado grupos homogéneos según la prueba de Duncan
                  </div>
                </div>
                <button 
                  className={styles.verInformeBtn}
